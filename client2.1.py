...
MONITOR_LOCK_FILE = "monitor_lock.tmp"  # 监控进程锁文件
        ...
        if os.path.exists(MONITOR_LOCK_FILE):
            try:
                ...
                if existing_pid and psutil.pid_exists(int(existing_pid)):
                    self.log("已存在运行中的监控进程，无需重复启动")
                    return
            except:
                self.log("监控进程锁文件损坏，将创建新的锁文件")
        
        # 创建当前监控进程的锁文件
        try:
            with open(MONITOR_LOCK_FILE, 'w') as f:
                f.write(str(os.getpid()))  # 写入当前进程PID
        except Exception as e:
            self.log(f"创建监控进程锁文件失败: {str(e)}")
...
MONITOR_LOCK_FILE = "{MONITOR_LOCK_FILE}"
...
def monitor_and_restart():
    ...
    while True:
        try:
            ...
            try:
                # 检查是否已有相同应用在运行
                current_app = os.path.basename(APP_PATH).lower()
                running_count = 0
                for p in psutil.process_iter(['name']):
                    if p.info['name'].lower() == current_app:
                        running_count += 1
                        if running_count > 1:  # 允许一个实例运行
                            print(f"已存在{{running_count}}个应用实例，无需重启")
                            cleanup(None, None)
                            return
                
                # 启动新实例
                if sys.argv[0].endswith('.exe'):
                    new_process = subprocess.Popen([APP_PATH], close_fds=True)
                else:
                    new_process = subprocess.Popen([sys.executable, APP_PATH], close_fds=True)
                
                print(f"应用已重启，新进程ID: {{new_process.pid}}")
                cleanup(None, None)  # 重启后监控进程退出
                return
            except Exception as e:
                print(f"重启失败: {{e}}")
                time.sleep(10)
    ...
    def stop(self):
        ...
        if self.server_socket:
            try:
                ...
                pass
        # 清理临时文件
        for file in ["process_monitor_temp.py", MONITOR_LOCK_FILE]:
